	...
	<div id="body">
		...
		<form action="" method="post" style="clear:both;padding-top:50px;height:600px;">
			<table id="table-1" style="width:1260px;" >
				<tr>
					<th>编号</th><th>活动标题</th><th>活动内容</th><th>创建时间</th><th>组织名</th><th>操作</th>
				</tr>
			   <c:forEach items="${list}" var="list">
			   		<tr style="text-align:center;padding:10px;height:55px;">
			   			<td>${list.id}</td>
			   			<td><a href="NewsByTypeTitleServlet?id=${list.id}">${list.title}</a></td>
			...
			<c:if test="${page>1}"><a href="/2SS/showByPageServlet?page=1">首页</a></c:if>
			<c:if test="${page==1}">上一页</c:if>
			<c:if test="${page>1}"><a href="/2SS/showByPageServlet?page=${page-1}">上一页</a></c:if>
			<c:if test="${page==maxpage}">下一页</c:if>
			<c:if test="${page<maxpage}"><a href="/2SS/showByPageServlet?page=${page+1}">下一页</a></c:if>
			<c:if test="${page==maxpage}">末页</c:if>
			<c:if test="${page<maxpage}"><a href="/2SS/showByPageServlet?page=${maxpage}">末页</a></c:if>
		</form>
			<div id="footing">
